...
def dp_dLfrac(x, alpha, beta, beta_red):
    return x**beta_red * (x**-alpha + x**-beta)**-1
...
def dN_dz(z,Lfrac_min_func,Lfrac_max_func,qlf,cosmo):
    ...
    #The observable fraction for a certain luminosity should be:
    Lstar = 10.**(qlf.log_Lstar(z))*qlf.Lstar_units
    beta_red = 0.082
    f46 = 0.260
    obs_frac = f46 * ((Lstar/(1e46*u.erg/u.s)).to(1.))**beta_red
    ...
    if Lfrac_min>Lfrac_max:
        return 0
    dN_dVc = quad(dp_dLfrac,Lfrac_min,Lfrac_max,args=(alpha,beta, beta_red))[0]
    dN_dVc *= phi_star_prime * obs_frac
