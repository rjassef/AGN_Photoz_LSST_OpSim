...
def dN_dz(z, m_bright, m_faint, lL_lam_obs_Mlim_func, LSSTfilter, cosmo):
    ...
    return_qlf_in_band(nu_rest, z, l_band_grid, l_bol_grid, phi_bol_grid)
...
"""
def Nqso(zmin, zmax, m_bright, m_faint, LSSTfilter, area=4.*np.pi*u.sr, Mi_lim=np.inf, cosmo=None):
    ...
    return area.to(u.sr).value * quad(dN_dz, zmin, zmax, args=(m_bright, m_faint, lL_lam_obs_Mlim_func, LSSTfilter, cosmo), epsabs=1e-1, epsrel=1e-3)[0]
