...
def load_SED(sed):
    ...
    if sed=='Richards06':
        ...
        qso_spec = Table.read(SED_path+"/Richards_06.dat",format='ascii.cds')
        ...
        qso_spec = Table.read(SED_path+"vandenberk_composite.txt",format='ascii.cds')
        wave=qso_spec['Wave']
...
if qlf_module == "Shen20":
    ...
    B_curve    = np.loadtxt(root_path+"/Filter_Curves/B_bessell.filter", skiprows=1)
...
for filter in filters:
    data = np.loadtxt(Filters_path+"LSST_LSST.{}.dat".format(filter), skiprows=1)
