...
header["/uband_depth/Table_ug_band_depth_WFDonly.txt"] = [
    "run",
    "u_depth_3sig",
    "u_depth_3sig_rank",
    "u_depth_3sig_area",
...
for z in ["1.0","2.0","3.0"]:
    tname = "/color_excess/Table_color_excess_z{}_WFDonly.txt".format(z)
    header[tname] = ["run"]
...
tname = "/mean_time_between_obs/Table_mean_nights_between_bands_WFDonly.txt"
header[tname] = ["run"]
for k in range(len(filters[:-1])):
...
tname = "/Quasar_Counts/Metric/Table_EM5_Nqso_i_WFD.txt"
header[tname] = [
    "run",
    "Nqso_i",
    "Nqso_i_rank",
...
master_table.write("metrics_database_WFDonly.fits", format='fits', overwrite=True)
